...
namespace scgFullBodyController
{
    public class AiController : MonoBehaviour
    {
        ...
        // New variable to set the target position
        public Transform targetLocation; // AI will move to this location
        ...
        private void Start()
        {
            ...
            {
                targetLocation = GameObject.FindWithTag("Target")?.transform;
            }
        }
        ...
        private void Update()
        {
            ...
            if (!overrideAttack)
            {
                ...
                if (!playerInSightRange && !playerInAttackRange) MoveToLocation();
        ...
        // New method for AI to move to a specified location
        public void MoveToLocation()
        {
            if (targetLocation != null)
            {
                // Only update the destination if the AI is not already at the target
                float distanceToTarget = Vector3.Distance(transform.position, targetLocation.position);

                // If the AI is within a certain threshold of the target, stop moving
                if (distanceToTarget > 1f) // Adjust the threshold as needed
                {
                    agent.SetDestination(targetLocation.position);
                }
                else
                {
                    agent.SetDestination(transform.position); // Stop the agent when it's close enough
                }
            }
        }
