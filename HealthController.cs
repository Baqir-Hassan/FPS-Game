...
namespace scgFullBodyController
{
    public class HealthController : MonoBehaviour
    {
        ...
        void Die()
        {
            WaveTimer timer = FindObjectOfType<WaveTimer>();
            Spawn spawner = FindObjectOfType<Spawn>();
            GameOver gameOverUI = FindObjectOfType<GameOver>();

            if (isAiOrDummy && levelProgression != null)
            {
                levelProgression.OnEnemyKilled();
            }
            ...
            if (!dontSpawnRagdoll)
            {
                ...
                if (!isAiOrDummy)
                {
                    ...
                    if (gameOverUI != null)
                    {
                        gameOverUI.DisplaySurvivalTime();
                    }
                    text.SetActive(true);
